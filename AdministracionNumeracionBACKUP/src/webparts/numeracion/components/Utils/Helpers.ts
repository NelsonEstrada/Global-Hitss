...
export class Helpers {
    ...
    public getByIdUser(ID: number): Promise<any> {
      return this.web.siteUsers.filter(`ID eq ${ID}`).get();
    }

    public getByUserfilter(filter: string): Promise<any> {
      return this.web.siteUsers.filter(filter).get();
    }
    ...
    /*public uploadFile(repository: string, file: File, name: string): Promise<any> {
      return this.web
        .getFolderByServerRelativeUrl(this.siteRelativeUrl+'/'+repository+'/')
      ...
      return new Promise((resolve, reject) => {
        const fileobject = this.web.getFolderByServerRelativeUrl(this.siteRelativeUrl+'/'+repository+'/');
            ...
            .then(res =>{
              if(fields){
                res.file.getItem().then((item) => {
                  this.updateFile(repository, item, fields, function(data:any){
                    resolve(data)
                  })
               })
               .catch(err => {
    ...
    public updateFile(list:string, item:any, fields:any, functionSuccess:any){
    ...
    public getListOverThreshold(ListName: string,select:string, filtro: string, adicionales: string, funcionexitosa?: any) {
      ...
      this.getItemsList(ListName,'ID', '','', {property : "ID", asc:false},1
          ...
          var IdFinal = 400000//(result[0] ? result[0].Id : 0);
    ...
    public loadingModal = {
      ...
      show: function(texto:string, funcionSuccess:any) {
          ...
          if(!jQuery('#loadingModal')[0]){
              ...
              jQuery('body').append(
                      '<style>'+
                          '#loadingModal {background: #0003;display:none;position: fixed;top: 0;left: 0;width: 100%;height: 100%;z-index: 120;}'+
                      ...
                      '<div id="loadingModal">'+
                          '<div class="contentModal">'+
                              ...
                              '<img src="https://claromovilco.sharepoint.com/inspiraprod/RL_PROD/dev/SiteAssets/Imagenes/LoadingGif.gif" class="imgModal"></img>'+
                          '</div>'+
                      '</div>')
          }
      ...
      hide: function(funcionSuccess?:any) {
